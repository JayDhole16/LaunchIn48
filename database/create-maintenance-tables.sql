-- Create maintenance tables for proper business tracking
...
-- 4. Maintenance Notifications/Reminders Table
CREATE TABLE IF NOT EXISTS maintenance_notifications (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  project_maintenance_id UUID NOT NULL REFERENCES project_maintenance(id) ON DELETE CASCADE,
  user_id UUID NOT NULL REFERENCES users(id) ON DELETE CASCADE,
  
  notification_type VARCHAR(30) NOT NULL CHECK (notification_type IN ('expiry_warning', 'expired', 'payment_due', 'payment_success')),
...
CREATE INDEX IF NOT EXISTS idx_project_maintenance_validity_end ON project_maintenance(total_validity_end);
